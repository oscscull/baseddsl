...
The thing to copy to start on **MariaDB**: the *same* `.bsl` schema + end-to-end scenario
as [`sqlite-quickstart`](../sqlite-quickstart), consumed through the **generated typed
client** over the in-process **`Engine`** — pointed at a real MariaDB server.
...
Then run the rest from this directory (`examples/mariadb-quickstart`).
...
```sh
docker rm -f based-mariadb   # then re-run the `docker run …` above
```
