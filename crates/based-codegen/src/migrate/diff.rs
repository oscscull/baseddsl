...
pub enum Step {
    ...
    /// `alter schema <table> <from> -> <to>` — a model moved SQL-schema / database
    /// namespace (its `@schema` changed; the table name is unchanged). Postgres `ALTER TABLE
    /// … SET SCHEMA`, MySQL/MariaDB cross-database `RENAME TABLE`; SQLite has no in-place
    /// cross-attached-database move, so it renders a loud raw-rebuild pointer. `None` on
    /// either side is the connection's default namespace.
    AlterSchema {
        table: String,
        from: Option<String>,
        to: Option<String>,
...
fn diff_table(prev: &TableSnap, now: &TableSnap, renames: &[Rename], steps: &mut Vec<Step>) {
    // A model moving `@schema` namespace: emit a reviewable schema-move step so the change
    // is never silently missed (the table keeps its name — its schema alone changed).
    if prev.schema != now.schema {
        steps.push(Step::AlterSchema {
            table: now.name.clone(),
            from: prev.schema.clone(),
            to: now.schema.clone(),
        });
    }
...
impl Step {
    ...
    pub fn table_name(&self) -> Option<&str> {
        match self {
            ...
            | Self::AlterSchema { table, .. } => Some(table),
