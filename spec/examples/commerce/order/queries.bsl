...
# the visible half of the both-sides contract (auth.md Handle 2).
...
# the standing scope explicitly — a greppable, linted escape hatch (auth.md). The
...
# param, so an order can't be placed into another tenant's scope (auth.md).
