...
`based gen sql` (the from-scratch full DDL) is unchanged and complementary:
...
the *resolved* schema (`CheckedSchema`) as of that migration. It is the baseline the next
...
indexes (declared + the inferred join-key baseline), soft-delete/created/updated roles, and
...
Serialization (`based-codegen::migrate`; not in `grammar.ebnf` as it is a
...
the default `id` is elided as an invariant. Illustrative shape:
...
The `id` column, being universally implicit, is elided from the per-table column list and
...
so the SQL matches the neutral step (principle 4).
...
`bool`, `json`); the renderer maps them per dialect. An **opaque** column or index
...
# relations = their FK column. A new/dropped/retyped relation is an
...
applying its steps to the previous snapshot; the diff engine guarantees the two agree.
...
- **Marked "not offline-verifiable" (principle 6, never silent).** A migration carrying any `raw`
  ...
  declared alongside it so the snapshot stays honest — **TODO:** decide the annotation form
...
**SQLite** (`text`→`TEXT`; indexes are separate `CREATE INDEX`):
```sql
ALTER TABLE `product` ADD COLUMN `barcode` TEXT NULL;
CREATE INDEX `idx_product_barcode` ON `product` (`barcode`);
```

**MariaDB** (`text`→`VARCHAR(255)`, backtick quoting):
```sql
ALTER TABLE `product` ADD COLUMN `barcode` VARCHAR(255) NULL;
CREATE INDEX `idx_product_barcode` ON `product` (`barcode`);
```

**Postgres** (`text`→`TEXT`, double-quote quoting, separate `CREATE INDEX`):
```sql
ALTER TABLE "product" ADD COLUMN "barcode" TEXT NULL;
CREATE INDEX "idx_product_barcode" ON "product" ("barcode");
```
...
- **A `down.mig` is honored only when it carries an executable statement.** It is **raw per-dialect
  SQL** (`;`-terminated statements — the honest form for a hand-written reverse): a
  neutral-vocabulary down would need a lossless neutral-step *text parser* the engine deliberately
  doesn't have (the up path is snapshot-authoritative, not text-parsed), and someone writing a
...
- **Down-invocation surface.** `based migrate apply --down` rolls back the single most-
...
- **`content_hash` = a stable hash of the canonical `up.mig`**
  ...
  same FNV family the runtime uses for request fingerprints; collision resistance is not
...
rest of the LSP uses, fully offline:
...
| `based migrate render [--number NNNN] [--dialect D]` | Render migrations' steps to per-dialect SQL and print it — the review-the-SQL step. `--number` picks one migration, else all in order; `--dialect` overrides the manifest target. Offline: re-derives each migration's structural steps as `diff(snapshot[N-1], snapshot[N])` from the stored `schema.snap`s (snapshot-authoritative), appends the authored `raw` steps, and **refuses** a migration whose structural `up.mig` lines diverge from the snapshot. Does not touch a DB. |
