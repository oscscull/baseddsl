...
An **optional context read** written at the use site as `$ctx.<field>?` (auth.md Handle 1) is related but
**not** the same lowering: an absent context field binds SQL NULL and its `=`/`!=` leaf becomes *null-safe*
(`col IS NULL`), matching the unset rows rather than widening to TRUE — because a widening auth leaf under
an `or` would leak the rows its co-guard gates. Unlike a `?` param (a marker on the *signature*), the `?`
there rides the `$ctx.<field>` reference itself, and is a query-filter construct only — never a scope, a
write, or a filter body (`E0339`).
...
- `in` takes one of three right-hand sides:
  - a **single bound value** (`status in $status` — one `$param`), equivalent to `=`;
  - a **parenthesized value list**: `status in (open, waiting, $other)`. List elements are ordinary values — literals, enum variants, `$param` references, columns — each checked against the left column's type (an enum column checks variant membership per element, `E0154`; a family mismatch like `total in (1, "x")` is the same error an `=` comparison gives). Lowers to SQL `IN (v, v, …)` with `$param` elements bound as parameters;
  - a **variable-length array param** — `col in $arr` where `$arr` is an array type (`text[]`, `int[]`, a relation-id array `Order[]`, …): the runtime-sized set of values, one placeholder per element. `find_in(vals: text[]) -> CardRow[] { list Card where (color_identity in $vals); }` filters against whatever list the caller passes. Also written as the param binding `vals: text[] in color_identity`. Each element is coerced against the column's scalar family (a `text[]` element must be text, an `int[]` element an int; a wrong-typed or non-array arg is a `bad_arg`). An **empty** array matches nothing (it lowers to `col IN (NULL)`, never the illegal `IN ()`). Combines with the optional `?` guard: `find_in(vals?: text[])` skips the filter entirely when `vals` is omitted, and matches nothing when it is present but empty.
