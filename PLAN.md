...
- **T2. decimal + float** — exact `decimal` (money) + `float`; new `Primitive`s through `sql_type` +
  `prim_family` + the runtime `SqlValue` decode arms (enum did *not* touch those — this does).
...
|-------|-------|------------------|
| based-ast | ✅ stable | AST mirrors grammar.ebnf node-for-node. No logic. `Decl::Enum` (variants carry an optional `= STRING\|INT` value) + `DefaultVal::Variant` (D82). |
...
| based-sema | ✅ stable | resolution + checks + lints + `CheckedSchema` IR (incl. `@was` rename directive on `RModel`/`RMember` + `E0190`/`E0191`, D67; enum decls → `REnum` with inferred `kind` (string/int) + valued variants, kind/dup/ordered-op/membership checks E0104/E0106/E0154/E0155/E0156/E0157/E0158, D82). Detailed behaviour in the next section. |
...
and `enum(…)`/`enum:int(…)` in the neutral snapshot so a variant or kind change diffs (D82); `migrate` → `schema.snap`/`up.mig` diff (D39) + `render_sql` per-dialect migration SQL (D41) + `sql_statements`/`content_hash` for apply (D42) + scope serialization (D50) + `@was` snapshot-authoritative renames (`Snapshot.renames` persisted → `rename table`/`rename column` steps → per-dialect `ALTER … RENAME`), the `raw(dialect)` escape step (`parse_raw_steps`), and the offline `drift` helper (D67). |
