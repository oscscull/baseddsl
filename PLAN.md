...
5. **Schema evolution: migration generation.** A `.bsl` schema change produces a reviewable,
   ...
   safely evolve a production database with is not adoptable. **The core is met (E1 spec, E2/D39 snapshot
   + diff, E3/D41 per-dialect render, E4/D42 apply + `_based_migrations` ledger + status/verify + raw-SQL
   `down.mig`), proven live against real MariaDB/SQLite.** Remaining: E5 (`@was` renames + the offline LSP
   drift diagnostic) + the `raw(dialect)` up step.
