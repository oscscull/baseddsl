...
- **NF11. ✅ done (D103, keystone). Inferred indexes + implicit `id` → explicit-in-source.**
  ...
  explicit `@index`/`id` lines (example migrations regenerated). Original resolution + writeup
  below. Resolution (owner 2026-07-21): a traversed-but-unindexed join key → error
...
## Track T — core DB feature parity (owner-approved 2026-07-09; at T6)

A confirmed 6-item queue closing the gap to a general DB-first DSL (commerce is only a *named*
example, not the domain). **T1–T5 done (enum D82, decimal/float D83, atomic update exprs D100,
aggregations+group-by+having D101, upsert + m2m-via-explicit-junction D102 — one m2m sugar slice
left open, see T5); next is T6 (referential actions).** Worked in order; each iteration
marks its item + D#. Tier-2 follow-ons below.
