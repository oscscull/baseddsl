...
**Track C — VS Code extension (DoD #3, independent, may run in parallel).**
  ...
  - **C4. 🔴 NEXT PRIORITY. Feature-parity audit + fill-in** (baseline editor features a `.bsl` author
    ...
    document symbols (D44), completion (D45), go-to-def (D43). **Remaining:** workspace symbols (`⌘T`),
    find-references + rename (rename needs the full reference-site index — a superset of the go-to-def
