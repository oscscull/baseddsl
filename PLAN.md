...
transactions). It is slice 2 of the transaction seam (D118 slice 1). Remaining tier-2: computed shape
fields; the SQLite incremental-FK rebuild and a `bytes` field inside a to-many array on SQLite are the
explicit deferrals. Batch-by-batch history is in `PLAN-archive.md`.
...
Tier-2 (remaining): computed shape fields. (`list distinct` D116, `time`/`bytes` D117 done;
`for update` locking now folded into the transaction-seam plan below, slice 2.)
