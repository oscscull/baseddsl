...
The read-decide-write seam mutations.md promised, spec'd in full in `spec/syntax/transactions.md`
and recorded in **D118**. An **embedded-only** (in-process) transaction ladder over the `Engine`:
the DSL stays non-Turing-complete (read-decide-write logic is host Rust against an engine-owned
transaction). Built in **3 slices**; the generated client is already `Client<T: Transport>`, so a
transaction is the same client over a transaction-bound transport — no per-callable codegen.
...
- **Slice 3. NOT STARTED. BYO `adopt` + flagship example.** `client::adopt(&mut caller_sqlx_tx)`
  binds the generated calls to a transaction the caller already opened on the same driver (per-driver
  adapters; caller commits), plus the axum-helpdesk read-decide-write use case. Documented.
