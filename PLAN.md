...
`@was` self-consuming gen + teach-at-checkpoint (D105), and **NF8 accurate snapshot-authoritative
...
OpenAPI unchanged, proven live on SQLite. **`time` + `bytes` scalar types are done (D117)** — `time` =
time-of-day (`TIME`/TEXT-on-SQLite, ordered like date/timestamp, `HH:MM:SS` wire string), `bytes` =
binary blob (`BYTEA`/`BLOB`, base64 wire string, equality-only); full pipeline incl. 3-driver binary
decode (Postgres `pg_time`/`bytea`→base64) + E0313/E0314, proven live on SQLite + Postgres + MariaDB.
**`for update` pessimistic locking is done (D119)** — a trailing block-body query modifier lowering to
`SELECT … FOR UPDATE` per dialect (no-op on SQLite), compile-time-confined to transaction clients via a
`TxBound` marker trait; E0315–E0318; proven live on Postgres (row lock held across concurrent
transactions). It is slice 2 of the transaction seam (D118 slice 1). **Computed shape fields are done
(D121)** — per-row derived scalars `out = <expr>`: arithmetic over the numeric family, `||` concatenation
...
- **PR4. ✅ DONE (D110 impl), `@key` deferred. PK generation-strategy axis — single-column.** The full
  ...
  `serial` DDL per dialect, wire representation (serial id = JSON number, OpenAPI `{type:integer}`, client
  ...
  **Proven live on all three dialects** (SQLite in-memory `serial_integration.rs`; live Postgres +
  ...
  - **Wire representation:** `Id<M>` repr is per-entity (String vs `i64`); serial ids are JSON numbers; OpenAPI
...
- **PR5. Representability audit + doc-truth pass (owner-raised 2026-07-24).** A dedicated systematic
  ...
  **PR5 RESULTS (audit ran 2026-07-24, 22 agents, adversarially verified; task wcpdg9k5s).** Top question
  ...
  - **PR8 ✅ DONE (miscompile, D112). `id UUID` rejected on `dialect="mysql"` and MariaDB < 10.7.**
    ...
    protocol — so the split is accurate-output + future-proofing). `sql_type` emits `CHAR(36)` for
...
- **NF5. ✅ done (D97). `Page<T>` carries `with count`'s total.** `Page<T>` gains
  ...
  has the field only then; `skip_serializing_if` keeps a re-served page wire-identical).
...
- **NF8. ✅ done (D106). Accurate snapshot-authoritative `up.mig` contract + a real editable surface.**
...
- **NF11. ✅ done (D103, keystone). Inferred indexes + implicit `id` → explicit-in-source.**
  ...
  **E0260** (was `W0103`) + one-key LSP autofix (`unindexed(…)` stays the visible opt-out); a model
  ...
  no declared `id`, become **compiler errors** (not IDE-only, so the CLI is equally strict)
...
## Track T — core DB feature parity (owner-approved 2026-07-09; T1–T6 done, tier-2 language items done)
...
- **T6. ✅ done (D108). FK referential actions** — opt-in DB `FOREIGN KEY` constraints with
  ...
  **divergence-reason rule** (spelled like `@no_id("reason")`): a reason is required exactly when a
  ...
  `add/drop foreign_key` step (PG/MariaDB `ALTER … ADD/DROP CONSTRAINT`; **SQLite alter-FK is a loud
...
- **Tier-2. Computed shape fields ✅ done (D121).** A `=`-valued shape field may project a
  per-row derived scalar `out = <expr>`: arithmetic (`+ - * /`) over the numeric family, `||`
  concatenation → text (per-dialect `Dialect::concat`: `||` on SQLite/Postgres, `CONCAT(…)` on
  the MySQL/MariaDB family), and `case when <pred> then <expr> else <expr> end` (the `when`
  reuses the shared predicate grammar/lowering; branches recurse). A dedicated `ShapeExpr` AST +
  `ShapeValue::Computed` (D100's mutation-SET arith untouched); the parser keeps a lone reach as
  `ShapeValue::Path`/flatten so every existing golden is byte-identical. Operands are the row's
  own columns / reached paths / literals / enum variants — **no params** (a shape has none →
  E0323). Type inferred from the expression (numeric promotion / text / unified CASE branch) and
  typed in client + OpenAPI via one shared `computed_result`. Reaches governed exactly like a
  `Path` — `scope.rs`'s shape walk recurses into a `Computed` field so a scoped/soft-deleted
  reached model is confined (no D81-class leak). Projection-only; not an aggregate (a computed
  field alongside an aggregate field → E0324). Codes E0320–E0324; runtime unchanged. Surface:
  AST/lexer/parser/grammar → sema → codegen (`Dialect::concat` + `dml`/`client`/`openapi`) → fmt
  → LSP (keywords + operand go-to-def/refs) → TextMate → `spec/syntax/shapes.md`. Tests: sema +/−
  (one per code), codegen SQL per dialect, client + OpenAPI typing, fmt round-trip, parser + sema
  conformance golden `computed_fields`, **live SQLite** `computed_integration.rs` (arithmetic /
...
**✅ COMPLETE (D118–D120).** The read-decide-write seam mutations.md promised, spec'd in full in
...
- **Slice 1. ✅ DONE (D118).** Isolation (`TxOptions`/`Isolation`/`AccessMode`, applied per dialect
  ...
  `SET TRANSACTION …` + `BEGIN`, SQLite `BEGIN DEFERRED/IMMEDIATE/EXCLUSIVE`); **rung 1** managed
  closure `client::transaction(&engine, opts, |tx| …)` (commit on Ok, rollback on Err/panic) +
  `transaction_retrying(…, Retry::on_serialization(max), …)` (re-runs the closure on a classified
  `Deadlock`); **rung 2** explicit handle `engine.begin(opts) -> Transaction` / `txn.client()` /
  `commit`/`rollback` (drop = rollback). Central refactor: `serve::dispatch_on` + `run_mutation_on`
  run a request against a **provided open `Tx`** (auto-commit path untouched; isolation rides a new
  `Db::begin_tx`); `TxTransport` wraps the `Box<dyn Tx>` in `Arc<tokio::sync::Mutex<…>>` for `Send`.
  Proven live (SQLite commit+rollback read-decide-write; Postgres isolation SET issued + rung-2
  round-trip); retry proven via an injected serialization failure.
- **Slice 2. ✅ DONE (D119). `for update` row locking.** A trailing `.bsl` block-body query modifier
  (`get Order where (…) for update`) lowering to `SELECT … FOR UPDATE` per dialect (Postgres + MySQL/
  MariaDB family; a no-op on SQLite, whose whole-database transaction lock already serializes writers),
  **compile-time-confined to transaction clients** via a `TxBound` marker trait — the locking methods live
  in `impl<T: Transport + TxBound> Client<T>` and only `TxTransport` implements `TxBound`, so calling one on
...
- **Slice 3. ✅ DONE (D120). BYO `adopt` + flagship example.** `client::adopt_<driver>(&engine, &mut
  ...
  begins/commits/rolls back** (the caller owns the boundary; dropping the adopted client leaves the tx
  untouched, unlike rung 2). Per-driver borrowed `DbRead` adapters (`AdoptedPg`/`AdoptedSqlite`/
  `AdoptedMaria`) wrapped in `AdoptedTransport<D>`, run through the same `dispatch_on` core (generalized
  from `Tx` to any `DbRead`, so engine-owned and adopted transactions share one path); `TxBound` so `for
  update` works through it; one `#[cfg]`-gated `adopt_*` constructor per compile-target dialect; `sqlx`
  re-exported from `based_runtime`. Proven live (raw + baseddsl write atomic on commit, discarded together
  on rollback) on Postgres + SQLite, and in the axum-helpdesk (`resolve_with_audit` → `POST
  /tickets/{id}/resolve`, in the smoke). The 3-rung seam is complete.
...
| based-parser | ✅ works | hand-written RD parser + lexer; golden + unit tests. Parses `decimal(p, s)` (optional args → `decimal_args`, bare = `(38, 9)`) + `float`; a `FLOAT` token → `Literal::Decimal` exact text (D83); an update assign RHS parses as a precedence-climbing arithmetic expression (`+ - * /`) over `value` leaves, new lexer tokens `+ - * /` (D100); a shape value `= count()`/`= sum(col)` parses to `ShapeValue::Agg`, and `group by (cols)`/`having (pred)` as query clauses (D101); a create's `on conflict (cols) update { … }` upsert tail (D102); field-level `@fk[("reason", on_delete: …, on_update: …)]` / `@no_fk[("reason")]` on a forward relation + model-level `@no_fk` (D108); `time`/`bytes` type keywords (D117); a trailing `for update` after the clause list in a block query body (D119); a computed shape-field expression `out = <expr>` — a precedence-climbing arithmetic/`||`-concat parser with a `case when … then … else … end` leaf (new `PipePipe` lexer token), a lone reach staying `ShapeValue::Path`/flatten (D121). |
...
| based-codegen | ✅ stable | `sql::ddl\|dml\|mutations` → dialect-aware DDL/SELECT/INSERT-UPDATE-DELETE (MariaDB/SQLite/Postgres, D28/D29) through one `Dialect` quoting/type seam; declared-shape re-select on every surviving write (create-keyed D12 + update/delete/restore where-keyed D58); nested to-one shape sub-objects (D55) + to-many nested arrays via correlated-subquery JSON aggregation incl. self-ref aliasing (D57), each with a nest-reached scoped child's `@scope` injected into the join `ON` / subquery `WHERE` (D81) + keyset-cursor pagination (lexicographic `WHERE` + hidden `__keyset_` columns, D56); `client` → typed Rust client (nested `Vec<…>` for to-many, paginated inputs carry a typed `cursor`/`offset`, D56/D57; **per-entity phantom-typed ids** `Id<entity::M>` — transparent wire, `from_raw` escape, no blanket `From<String>`, D70; an opaque **`Cursor`** newtype for the keyset surface (single `#[serde(transparent)]` type, `from_raw` escape, D73); a structurally-sound **`ClientError`** — a `kind`(Transport/Decode/Api{status,code})-carrying `std::error::Error` with `Display`+`source()` (Arc-backed, stays `Clone`) + `code()`/`status()`/`message()` accessors, the embedded bridge preserving the wire status+code+message, D71) with an **opt-in in-process embedded bridge** (`ClientOptions::embedded` / `client_with` → emits `client::embedded(&engine)` over `based_runtime::Engine`, so an embedder writes zero `Transport` plumbing; referenced by path, no based-runtime dep; D62); the embedded bridge also carries the **read-decide-write transaction seam** — `impl Transport for TxTransport`, the managed `client::transaction`/`transaction_retrying`, `TxError`/`Retry`, and `TransactionExt::client()` (D118, slice 1); the `for update` locking read (D119, slice 2) — `Dialect::for_update_clause` seam emits `SELECT … FOR UPDATE` (no-op on SQLite) after ORDER BY/LIMIT, and the generated client confines the locking methods to `impl<T: Transport + TxBound> Client<T>` (a `TxBound` marker trait only `TxTransport` implements → a compile error on `Client<Embedded>`/wire); the BYO `adopt` interop rung (D120, slice 3) — `impl<D: DbRead> Transport for AdoptedTransport<D>` (one impl per driver, keyed-door runs on the tx, streaming-door rejected) + its `TxBound` impl (so `for update` works through an adopted client) + exactly one `#[cfg]`-gated `adopt_<driver>` constructor for the compile-target dialect (`ClientOptions.dialect`); a `Dialect::concat` seam (`||` on SQLite/Postgres, `CONCAT(…)` on the MySQL/MariaDB family) + `Select::shape_expr` lower a computed shape field `out = <expr>` (arithmetic / concat / `CASE WHEN … END` reusing the shared predicate lowering) to one SELECT-list column; `computed_result` infers the field type (numeric promotion / text / unified CASE branch) for the client + OpenAPI, both reading it so they can't drift (D121); `openapi` → OpenAPI 3.1 (D24; an enum field → a string- or integer-schema with `enum: [...]`, D82); enum columns →
