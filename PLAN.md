...
**T4 — aggregations + group-by + having — is now done (D101)**; Track T continues at T5 (m2m + upsert).
...
example, not the domain). **T1–T4 done (enum D82, decimal/float D83, atomic update exprs D100,
aggregations+group-by+having D101); next is T5 (m2m + upsert).** Worked in order; each iteration
